...
\begin{figure}
  ...
  \end{center}
  \caption{Left: simulated voltage using parameters trained using guarino loss.
...
To evaluate the performance of the two loss functions, we measured spike timing prediction quality using the coincidence factor $\Gamma$ from Jolivet et al.~\cite{Jolivet2008},
with a coincidence window $\Delta = 2$\,ms.
We found that for both training approaches (\ac{MSE}-based and guarino-based) could not produce meaningful parameter sets,
both produced parameter sets having a measured spike timing prediction quality of $\Gamma \approx 0$.
...
We successfully implemented the \ac{AdEx} model in Jaxley and enable automatic differentiation through surrogate gradients.
...
The main contribution was to implement a differentiable AdEx model Jaxley.
We gained an understanding of loss requirements, and will iterate on this in the future.
...
I used Anthropics Opus 4.5 via Claude Code.

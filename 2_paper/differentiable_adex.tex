...
\end{IEEEkeywords}
\mtodo[cyan]{UPDATE KEYWORDS: Consider adding ``Surrogate Gradients'', ``Parameter Estimation'', ``Loss Function Design'', ``Differentiable Simulation''. Remove ``GPU-Acceleration'' if that's not a main contribution anymore.}
...
\mtodo[red]{DELETE this old framing about ``performance gains'' - the new question is about \emph{feasibility} and \emph{requirements}, not speed comparison}
